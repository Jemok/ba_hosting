    ...
    <div class="top_menu">
        ...
        <div class="title">
            ...
            <a href="{{ url('innovator/profile/'.$thread->messages()->with('user')->oldest()->first()->user->hash_id) }}">
                started by:{{\Auth::user()->id == $thread_mother->messages()->with('user')->oldest()->first()->user->id ? 'Me' : $thread->messages()->with('user')->oldest()->first()->user->first_name. ' -
                (Innovator)' }}
            </a>
            @endif
            ...
            about <a href="{{ url('innovation/'.$thread_mother->innovation->id) }}">{{$thread->innovation->innovationTitle}}</a>

        </div>
    </div>

    <ul class="messages"  id="thread_{{$thread_mother->id}}" data-id="{{ $thread_mother->id }}">
        @foreach($thread->messages()->oldest()->get() as $message)
        @include('messenger.html-message-one', $message)
        @endforeach
    </ul>
